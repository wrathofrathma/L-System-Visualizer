...
class GraphObject(QuaternionObject):
    def __init__(self, dimensions=2):
        ...
        self.data_initialized=False # Flag for whether we've actually set the graph data. 
        self.opengl_initialized=False # Flag for whether we've initalized the opengl objects
        self.update=True # Flag for whether we need to update our data on teh graphics card. 
        self.vertices = []
        self.colors = []
        self.indices = []
    ...
    def init_ogl(self):
        ...
        if(len(self.indices)==0):
            print("[ ERROR ] Attempting to initialize an indexed vertex object with no indices.")
            exit(1)
