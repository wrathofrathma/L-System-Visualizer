...
class Glossary(QWidget):
  ...
  def create_labels(self):
    # creates the labels to be displayed
    ...
    self.little_h = QLabel('h: moves forward in a line of half unit length.')
    ...
    self.left_bracket = QLabel('[: starts a branch. Must have a ] somewhere after.')
    self.right_bracket = QLabel(']: ends a branch. Must have a [ somewhere before.')
    self.left_parenthesis = QLabel('(: decreases the angle by a turning angle.')
    self.right_parenthesis = QLabel('): increses the angle by a turning angle.')
    self.left_angle = QLabel('<: divides line length by the length factor.')
    self.right_angle = QLabel('>: multiplies the line length by the length factor.')
