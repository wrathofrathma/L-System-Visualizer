...
class QuaternionObject:
    ...
    def get_y_axis(self):
        rotation = self.get_rotation_quat()
        return conjugate(rotation) * vec3(
            0, 1, 0
        )  # Determines what the object's z axis is relative to itself.
    ...
    def translate(self, offset, relative=True):
        offset = vec3(
            offset
        )  # making sure we're using a vec3. It is fine for python lists, numpy arrays, and glm::vec3
        if relative:
    ...
    def generate_model_matrix(self):
        ...
        self.model_matrix = trans_matrix * rot_matrix * scale_matrix
        # return mat4(1)
